    ...
    function handleModal() {
        if (interaction.type === "modal" && interaction.target) {
            ...
            if (targetElement) {
                ...
                console.warn(
                    `Modal-Ziel mit ID "${interaction.target}" nicht gefunden.`
                );
            }
        }
    }
...
{:else if interaction.type === "submit"}
    <button type="submit" class={buttonClasses} on:click>
        {#if icon.position === "left"}
            <IconWrapper
                name={icon.name}
                size="18"
...
{:else if interaction.type === "modal"}
    <button class={buttonClasses} on:click={handleModal}>
        {#if icon.position === "left"}
            <IconWrapper
                name={icon.name}
                size="18"
...
{:else if interaction.type === "close"}
    <button class={buttonClasses} on:click={closeModal}>
        {#if icon.position === "left"}
            <IconWrapper
                name={icon.name}
                size="18"
